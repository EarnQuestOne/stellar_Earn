...
export class AddDisputes1850000000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."disputes_status_enum" AS ENUM('PENDING','UNDER_REVIEW','RESOLVED','APPEALED','WITHDRAWN')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_disputes_submission" ON "disputes" ("submissionId")`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_disputes_initiator_status" ON "disputes" ("initiatorAddress", "status")`,
    );
  }

  async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX "public"."idx_disputes_initiator_status"`,
    );
