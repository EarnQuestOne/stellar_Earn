...
const {
  ...
  const mockSetWallet = vi.fn(),
    mockDisconnectKit = vi.fn().mockResolvedValue(undefined);
...
function Harness() {
  ...
  return (
    <>
      <button data-testid="connect" onClick={() => w.connect('freighter')}>
        c
      </button>
...
describe('WalletProvider — connect/disconnect', () => {
  beforeEach(() => {
    ...
    Object.assign(
      mockState,
      { address: null, selectedWalletId: null, walletError: null },
      mockActions
    );
  });
  it('connect() calls the kit and stores the resolved address', async () => {
    ...
    await waitFor(() =>
      expect(mockActions.setWalletAddress).toHaveBeenCalledWith('GNEW')
    );
  });
  it('disconnect() clears store state after a prior connect', async () => {
    ...
    fireEvent.click(getByTestId('connect'));
    await waitFor(() =>
      expect(mockActions.setWalletAddress).toHaveBeenCalled()
    );
