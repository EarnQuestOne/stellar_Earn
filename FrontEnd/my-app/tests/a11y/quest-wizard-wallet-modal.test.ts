...
test.describe('Accessibility: quest wizard + wallet connect modal', () => {
  test('quest creation wizard has no critical axe violations', async ({
    page,
  }) => {
    await expectAxeToPass({ page, url: '/en/quests/create' });
  });

  test('quest wizard remains accessible after advancing a step', async ({
    page,
  }) => {
  ...
  test('wallet connect modal has no critical axe violations when opened', async ({
    page,
  }) => {
  ...
  test('wallet connect modal closes on Escape without trapping focus permanently', async ({
    page,
  }) => {
    await page.goto('/en', { waitUntil: 'networkidle' });
    const connectButton = page
