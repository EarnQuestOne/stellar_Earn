...
const STATUS_BADGE: Record<PayoutStatus, string> = {
  ...
  retry_scheduled: 'bg-orange-50 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300',
};
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const sorted = [...res.payouts].sort((a, b) => {
        const va = sortField === 'amount' ? a.amount : new Date(a.createdAt).getTime();
        const vb = sortField === 'amount' ? b.amount : new Date(b.createdAt).getTime();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">Payout History</h3>
        {total > 0 && <span className="text-xs text-zinc-500 dark:text-zinc-400">{total} total</span>}
      </div>
      ...
      {payouts.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 rounded-xl border-2 border-dashed border-zinc-200 dark:border-zinc-800 text-center">
          <p className="text-sm font-medium text-zinc-500 dark:text-zinc-400">No payout history yet</p>
        ...
        <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-800/50">
                  <tr>
                    <th className="px-4 py-3 font-semibold text-zinc-900 dark:text-zinc-50 cursor-pointer hover:text-primary" onClick={() => toggleSort('createdAt')}>Date</th>
                    <th className="px-4 py-3 font-semibold text-zinc-900 dark:text-zinc-50 cursor-pointer hover:text-primary" onClick={() => toggleSort('amount')}>Amount</th>
                    <th className="px-4 py-3 font-semibold text-zinc-900 dark:text-zinc-50">Asset</th>
                    <th className="px-4 py-3 font-semibold text-zinc-900 dark:text-zinc-50">Status</th>
                    <th className="px-4 py-3 font-semibold text-zinc-900 dark:text-zinc-50">Tx Hash</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                  {payouts.map((p) => (
                    <tr key={p.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                      <td className="px-4 py-4 text-zinc-600 dark:text-zinc-400">{date(p.createdAt, 'medium')}</td>
                      <td className="px-4 py-4 font-medium text-zinc-900 dark:text-zinc-50">
                        {reward(p.amount, { type: 'custom', label: { singular: p.asset, plural: p.asset } })}
                      </td>
                      <td className="px-4 py-4 text-zinc-600 dark:text-zinc-400">{p.asset}</td>
                      ...
                      <td className="px-4 py-4">
                        {p.transactionHash ? (
                          <code className="text-xs text-zinc-400 truncate max-w-[7rem]">{p.transactionHash}</code>
                        ) : (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between text-sm">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="rounded-lg border border-zinc-200 px-3 py-1.5 text-zinc-600 hover:bg-zinc-50 disabled:opacity-40 dark:border-zinc-700 dark:text-zinc-400">Previous</button>
              <span className="text-zinc-500 dark:text-zinc-400">Page {page} of {totalPages}</span>
              <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="rounded-lg border border-zinc-200 px-3 py-1.5 text-zinc-600 hover:bg-zinc-50 disabled:opacity-40 dark:border-zinc-700 dark:text-zinc-400">Next</button>
            </div>
          )}
        </>
      )}
