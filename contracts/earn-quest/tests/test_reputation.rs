...
fn test_get_unknown_badge_type_rejected() {
    let env = Env::default();
    env.mock_all_auths();
...
fn test_register_duplicate_badge_type_overwrites() {
    ...
    let bt = BadgeType {
        id: rookie_id.clone(),
        name: SString::from_str(&env, "Rookie Updated"),
        description: SString::from_str(&env, "Updated description."),
        xp_reward: 99,
    };
    client.register_badge_type(&admin, &bt);

    let updated = client.get_badge_type(&rookie_id);
    assert_eq!(updated.xp_reward, 99);
}

#[test]
fn test_update_badge_type() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let updated = client.get_badge_type(&id);
    assert_eq!(updated.xp_reward, 15);
}
...
fn test_remove_badge_type() {
    ...
    let res = client.try_get_badge_type(&id);
    assert!(res.is_err());
}
